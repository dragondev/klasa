...
module.exports = class extends Serializer {
	...
	async deserialize(data, entry, language, guild) {
		...
		const role = this.constructor.regex.role.test(data) ? guild.roles.cache.get(this.constructor.regex.role.exec(data)[1]) : guild.roles.find(rol => rol.name === data) || null;
		if (role) return role;
